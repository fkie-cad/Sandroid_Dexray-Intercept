📦
691831 /agent/hooking_profile_loader.js
✄
...
function hookError(context, error) {
  const normalized = ensureError(error);
  devlog(`[HOOK ERROR] [${context}] ${normalized.message}`);
}

// agent/utils/safe_native.ts
var retained = [];
function safeResolveExport(moduleName, exportName, context) {
  try {
    const address = moduleName === null ? Module.findGlobalExportByName(exportName) : Process.findModuleByName(moduleName)?.findExportByName(exportName) ?? null;
    if (address === null) {
      hookError(`${context}:${exportName}`, new Error(`Native export not found: ${exportName}` + (moduleName ? ` in ${moduleName}` : " (global)")));
      return null;
    }
    return address;
  } catch (error) {
...
function safeNativeFunction(address, retType, argTypes, context, options) {
...
function wrapInvocationCallback(context, phase, fn) {
  return function(arg) {
    try {
      fn.call(this, arg);
    } catch (error) {
...
function safeAttach(target, context, callbacks) {
...
function safeAttachExport(moduleName, exportName, context, callbacks) {
  const address = safeResolveExport(moduleName, exportName, context);
  if (!address)
    return null;
  return safeAttach(address, `${context}:${exportName}`, callbacks);
}
function safeReplace(target, context, retType, argTypes, replaceLogic) {
  if (!target || target.isNull()) {
  ...
  try {
    ...
    const replacement = new NativeCallback(
      function(...args) {
        try {
          ...
          hookError(context, error);
          return original ? original(...args) : void 0;
        }
      },
    ...
    const originalPtr = typeof Interceptor.replaceFast === "function" ? Interceptor.replaceFast(target, replacement) : (Interceptor.replace(target, replacement), target);
...
function safeEnumerateMatches(query, context) {
  try {
...
function stripModulePrefix(matchName) {
...
// agent/utils/safe_java.ts
function safePerform(context, fn) {
  if (!Java.available) {
...
function safeUse(className, context) {
  try {
...
function safeOverload(method, context, ...signatures) {
  if (!method) {
    hookError(`${context}:overload[${signatures.join(", ")}]`, new Error("Method is null or undefined"));
    return null;
  }
  try {
...
function safeImplementation(context, original, hookLogic) {
  return function(...args) {
    try {
...
function safeDeferred(context, fn) {
  return function() {
    try {
      fn();
    } catch (error) {
...
function hook_java_sql() {
  setImmediate(safeDeferred("database:hook_java_sql", () => {
    safePerform("database:hook_java_sql", () => {
      const sqliteDatabase = safeUse("android.database.sqlite.SQLiteDatabase", "database:hook_java_sql");
      if (!sqliteDatabase)
        return;
...
function hook_SQLCipher() {
  setImmediate(safeDeferred("database:hook_SQLCipher", () => {
    safePerform("database:hook_SQLCipher", () => {
      const SQLiteOpenHelper = safeUse("net.sqlcipher.database.SQLiteOpenHelper", "database:hook_SQLCipher");
      if (SQLiteOpenHelper) {
      ...
      const SQLiteDatabase = safeUse("net.sqlcipher.database.SQLiteDatabase", "database:hook_SQLCipher");
      if (!SQLiteDatabase) {
        return;
      }
...
function hook_room_library() {
  setImmediate(safeDeferred("database:hook_room_library", () => {
    safePerform("database:hook_room_library", () => {
      ...
      const SQLiteDatabase = safeUse("net.sqlcipher.database.SQLiteDatabase", "database:hook_room_library");
      if (SQLiteDatabase) {
        SQLiteDatabase.openOrCreateDatabase.overload("java.io.File", "java.lang.String").implementation = function(file, password) {
      ...
      const SupportSQLiteOpenHelper_Callback = safeUse("androidx.sqlite.db.SupportSQLiteOpenHelper$Callback", "database:hook_room_library");
      ...
      const SupportSQLiteDatabase = safeUse("androidx.sqlite.db.SupportSQLiteDatabase", "database:hook_room_library");
      ...
      const FlowCollector = safeUse("kotlinx.coroutines.flow.FlowCollector", "database:hook_room_library");
      if (FlowCollector) {
...
function hook_wcdb() {
  setImmediate(safeDeferred("database:hook_wcdb", () => {
    safePerform("database:hook_wcdb", () => {
      const wcdbDatabase = safeUse("com.tencent.wcdb.database.SQLiteDatabase", "database:hook_wcdb");
      if (!wcdbDatabase) {
        return;
      }
...
function copy_file(PROFILE_HOOKING_TYPE23, source, destinationPath) {
  var filename = get_filename(source);
  var destination = destinationPath + "/" + filename;
  Java.perform(function() {
    am_send(PROFILE_HOOKING_TYPE23, "creating local copy of unpacked file");
    const File2 = Java.use("java.io.File");
    ...
    if (sourceFile.exists() && sourceFile.canRead()) {
      ...
      fis.close();
      fos.close();
      am_send(PROFILE_HOOKING_TYPE23, "dumped successfully @ " + destination + "\n");
    } else {
      am_send(PROFILE_HOOKING_TYPE23, "file has already been deleted");
    }
  });
}
...
function install_system_library_hooks() {
  ...
  safePerform("load_library:install_system_library_hooks", () => {
    const SystemDef = safeUse("java.lang.System", "load_library:install_system_library_hooks");
    if (!SystemDef)
      return;
    const SystemLoad_1 = safeOverload(SystemDef.load, "load_library:System.load", "java.lang.String");
    const SystemLoad_2 = safeOverload(SystemDef.loadLibrary, "load_library:System.loadLibrary", "java.lang.String");
    if (SystemLoad_1) {
      SystemLoad_1.implementation = safeImplementation("load_library:System.load", SystemLoad_1, function(original, library) {
    ...
    if (SystemLoad_2) {
      SystemLoad_2.implementation = safeImplementation("load_library:System.loadLibrary", SystemLoad_2, function(original, library) {
...
function install_runtime_library_hooks() {
  ...
  safePerform("load_library:install_runtime_library_hooks", () => {
    const RuntimeDef = safeUse("java.lang.Runtime", "load_library:install_runtime_library_hooks");
    if (!RuntimeDef)
      return;
    const RuntimeLoad_1 = safeOverload(RuntimeDef.load, "load_library:Runtime.load", "java.lang.String");
    const RuntimeLoad_2 = safeOverload(RuntimeDef.loadLibrary, "load_library:Runtime.loadLibrary", "java.lang.String");
    if (RuntimeLoad_1) {
      RuntimeLoad_1.implementation = safeImplementation("load_library:Runtime.load", RuntimeLoad_1, function(original, library) {
    ...
    if (RuntimeLoad_2) {
      RuntimeLoad_2.implementation = safeImplementation("load_library:Runtime.loadLibrary", RuntimeLoad_2, function(original, library) {
...
function hook_shared_preferences() {
  ...
  safePerform("shared_prefs:hook_shared_preferences", () => {
    ...
    if (SharedPrefs) {
      const sharedPrefsInit = safeOverload(SharedPrefs.$init, "shared_prefs:SharedPreferencesImpl.$init", "java.io.File", "int");
      if (sharedPrefsInit) {
        sharedPrefsInit.implementation = safeImplementation("shared_prefs:SharedPreferencesImpl.$init", sharedPrefsInit, function(original, file, mode) {
    ...
    if (SharedPrefsEditor) {
      const putString = safeOverload(SharedPrefsEditor.putString, "shared_prefs:SharedPreferencesImpl$EditorImpl.putString", "java.lang.String", "java.lang.String");
      if (putString) {
        putString.implementation = safeImplementation("shared_prefs:SharedPreferencesImpl$EditorImpl.putString", putString, function(original, key, value) {
      ...
      editorMethods.forEach(({ method, args }) => {
        const overload = safeOverload(SharedPrefsEditor[method], `shared_prefs:SharedPreferencesImpl$EditorImpl.${method}`, ...args);
        if (overload) {
          overload.implementation = safeImplementation(`shared_prefs:SharedPreferencesImpl$EditorImpl.${method}`, overload, function(original, key, value) {
...
function hook_datastore() {
  ...
  safePerform("shared_prefs:hook_datastore", () => {
    const DataStore = safeUse("androidx.datastore.core.DataStore", "shared_prefs:hook_datastore");
    if (DataStore && DataStore.updateData && DataStore.updateData.overloads) {
      DataStore.updateData.overloads.forEach((overload, index) => {
        overload.implementation = safeImplementation(`shared_prefs:DataStore.updateData[${index}]`, overload, function(original, ...args) {
    ...
    if (DataStore && DataStore.getData) {
      const getData = safeOverload(DataStore.getData, "shared_prefs:DataStore.getData");
      if (getData) {
        getData.implementation = safeImplementation("shared_prefs:DataStore.getData", getData, function(original) {
    ...
    const Preferences = safeUse("androidx.datastore.preferences.core.Preferences", "shared_prefs:hook_datastore");
    if (Preferences) {
      const prefsGet = safeOverload(Preferences.get, "shared_prefs:Preferences.get", "androidx.datastore.preferences.core.Preferences$Key");
      if (prefsGet) {
        prefsGet.implementation = safeImplementation("shared_prefs:Preferences.get", prefsGet, function(original, key) {
    ...
    const MutablePreferences = safeUse("androidx.datastore.preferences.core.MutablePreferences", "shared_prefs:hook_datastore");
    if (MutablePreferences) {
      const mutableGet = safeOverload(MutablePreferences.get, "shared_prefs:MutablePreferences.get", "androidx.datastore.preferences.core.Preferences$Key");
      if (mutableGet) {
        mutableGet.implementation = safeImplementation("shared_prefs:MutablePreferences.get", mutableGet, function(original, key) {
          const value = original.call(this, key);
          createSharedPrefEvent("datastore_prefs.get", {
            method: "get",
            key: key ? key.toString() : "unknown",
    ...
    const PreferencesKey = safeUse("androidx.datastore.preferences.core.Preferences$Key", "shared_prefs:hook_datastore");
    if (PreferencesKey) {
      const keyInit = safeOverload(PreferencesKey.$init, "shared_prefs:Preferences$Key.$init", "java.lang.String");
      if (keyInit) {
        keyInit.implementation = safeImplementation("shared_prefs:Preferences$Key.$init", keyInit, function(original, keyName) {
...
function hookGetIntent(original) {
  ...
  setTimeout(safeDeferred("intents:intent_hooks", () => {
    safePerform("intents:intent_hooks", () => {
      ...
      if (Intent) {
        ...
        getDataRef.implementation = safeImplementation("intents:Intent.getData", getDataRef, hookGetData);
      }
      const Activity = safeUse("android.app.Activity", "intents:intent_hooks");
      if (Activity) {
        const getIntentRef = Activity.getIntent;
        getIntentRef.implementation = safeImplementation("intents:Activity.getIntent", getIntentRef, hookGetIntent);
      }
    });
  }), 0);
}
...
function hook_broadcasts() {
  safePerform("broadcast:hook_broadcasts", () => {
    ...
    if (ContextWrapper.sendBroadcast) {
      const sendBroadcast1 = safeOverload(ContextWrapper.sendBroadcast, "broadcast:ContextWrapper.sendBroadcast", "android.content.Intent");
      if (sendBroadcast1) {
        sendBroadcast1.implementation = safeImplementation("broadcast:ContextWrapper.sendBroadcast[Intent]", sendBroadcast1, function(original, intent) {
      ...
      const sendBroadcast2 = safeOverload(ContextWrapper.sendBroadcast, "broadcast:ContextWrapper.sendBroadcast", "android.content.Intent", "java.lang.String");
      if (sendBroadcast2) {
        sendBroadcast2.implementation = safeImplementation("broadcast:ContextWrapper.sendBroadcast[Intent,String]", sendBroadcast2, function(original, intent, receiverPermission) {
          const intentInfo = getIntentInfo(intent);
          createBroadcastEvent("broadcast.sent", {
            class: "android.content.ContextWrapper",
            method: "sendBroadcast",
    ...
    if (ContextWrapper.sendStickyBroadcast) {
      const sendSticky = safeOverload(ContextWrapper.sendStickyBroadcast, "broadcast:ContextWrapper.sendStickyBroadcast", "android.content.Intent");
      if (sendSticky) {
        sendSticky.implementation = safeImplementation("broadcast:ContextWrapper.sendStickyBroadcast[Intent]", sendSticky, function(original, intent) {
          const intentInfo = getIntentInfo(intent);
    ...
    if (ContextWrapper.startActivity) {
      const startActivity1 = safeOverload(ContextWrapper.startActivity, "broadcast:ContextWrapper.startActivity", "android.content.Intent");
      if (startActivity1) {
        startActivity1.implementation = safeImplementation("broadcast:ContextWrapper.startActivity[Intent]", startActivity1, function(original, intent) {
          const intentInfo = getIntentInfo(intent);
      ...
      const startActivity2 = safeOverload(ContextWrapper.startActivity, "broadcast:ContextWrapper.startActivity", "android.content.Intent", "android.os.Bundle");
      if (startActivity2) {
        startActivity2.implementation = safeImplementation("broadcast:ContextWrapper.startActivity[Intent,Bundle]", startActivity2, function(original, intent, bundle) {
          const intentInfo = getIntentInfo(intent);
          createBroadcastEvent("activity.started", {
            class: "android.content.ContextWrapper",
            method: "startActivity",
    ...
    if (ContextWrapper.startService) {
      ...
      startServiceRef.implementation = safeImplementation("broadcast:ContextWrapper.startService", startServiceRef, function(original, service) {
    ...
    if (ContextWrapper.stopService) {
      ...
      stopServiceRef.implementation = safeImplementation("broadcast:ContextWrapper.stopService", stopServiceRef, function(original, name) {
    ...
    if (ContextWrapper.registerReceiver) {
      const registerReceiver1 = safeOverload(ContextWrapper.registerReceiver, "broadcast:ContextWrapper.registerReceiver", "android.content.BroadcastReceiver", "android.content.IntentFilter");
      if (registerReceiver1) {
        registerReceiver1.implementation = safeImplementation("broadcast:ContextWrapper.registerReceiver[BroadcastReceiver,IntentFilter]", registerReceiver1, function(original, receiver, filter) {
          return original.call(this, receiver, filter);
        });
      }
      const registerReceiver2 = safeOverload(ContextWrapper.registerReceiver, "broadcast:ContextWrapper.registerReceiver", "android.content.BroadcastReceiver", "android.content.IntentFilter", "java.lang.String", "android.os.Handler");
      if (registerReceiver2) {
        registerReceiver2.implementation = safeImplementation("broadcast:ContextWrapper.registerReceiver[BroadcastReceiver,IntentFilter,String,Handler]", registerReceiver2, function(original, receiver, filter, broadcastPermission, scheduler) {
...
function install_aes_secrets() {
  ...
  safePerform("aes:install_aes_secrets", () => {
    ...
    if (skInit1) {
      skInit1.implementation = safeImplementation("aes:SecretKeySpec.$init[byte[],String]", skInit1, function(original, keyBytes, algorithm) {
    ...
    if (skInit2) {
      skInit2.implementation = safeImplementation("aes:SecretKeySpec.$init[byte[],int,int,String]", skInit2, function(original, keyBytes, offset, length, algorithm) {
        createAESEvent("crypto.key.creation", {
          algorithm,
          key_length: length,
    ...
    if (ivInit) {
      ivInit.implementation = safeImplementation("aes:IvParameterSpec.$init[byte[]]", ivInit, function(original, ivBytes) {
...
function install_aes_keys() {
  ...
  safePerform("aes:install_aes_keys", () => {
    ...
    if (cipherInit1) {
      cipherInit1.implementation = safeImplementation("aes:Cipher.init[int,Key]", cipherInit1, function(original, opmode, key) {
    ...
    if (cipherInit2) {
      cipherInit2.implementation = safeImplementation("aes:Cipher.init[int,Key,AlgorithmParameterSpec]", cipherInit2, function(original, opmode, key, params) {
        const cipherId = this.hashCode();
        const keyBytes = key.getEncoded();
        activeCipherSessions.set(cipherId, {
          id: cipherId,
...
function install_aes_info() {
  ...
  safePerform("aes:install_aes_info", () => {
    ...
    if (doFinalBytes) {
      doFinalBytes.implementation = safeImplementation("aes:Cipher.doFinal[byte[]]", doFinalBytes, function(original, inputBytes) {
    ...
    updateMethods.forEach((method, index) => {
      method.implementation = safeImplementation(`aes:Cipher.update[${index}]`, method, function(original, ...args) {
        const cipherId = this.hashCode();
        const session = activeCipherSessions.get(cipherId);
        if (session) {
          createAESEvent("crypto.cipher.update", {
            ...
            operation_mode: session.opmode,
            update_call: index + 1
            // index captured from forEach closure
          });
        }
        return original.apply(this, args);
      });
    ...
    doFinalMethods.forEach((method, index) => {
      method.implementation = safeImplementation(`aes:Cipher.doFinal[${index}]`, method, function(original, ...args) {
...
function install_base64_hooks() {
  ...
  safePerform("encodings:install_base64_hooks", () => {
    ...
    if (decodeStrInt) {
      decodeStrInt.implementation = safeImplementation("encodings:Base64.decode[String,int]", decodeStrInt, function(original, str, flag) {
    ...
    if (decodeByteInt) {
      decodeByteInt.implementation = safeImplementation("encodings:Base64.decode[byte[],int]", decodeByteInt, function(original, input, flag) {
    ...
    if (decodeByteIntIntInt) {
      decodeByteIntIntInt.implementation = safeImplementation("encodings:Base64.decode[byte[],int,int,int]", decodeByteIntIntInt, function(original, input, offset, len, flags) {
    ...
    if (encodeByteInt) {
      encodeByteInt.implementation = safeImplementation("encodings:Base64.encode[byte[],int]", encodeByteInt, function(original, input, flags) {
    ...
    if (encodeByteIntIntInt) {
      encodeByteIntIntInt.implementation = safeImplementation("encodings:Base64.encode[byte[],int,int,int]", encodeByteIntIntInt, function(original, input, offset, len, flags) {
        const result = original.call(this, input, offset, len, flags);
        if (input.length !== 0) {
          const stack = threadInstance.currentThread().getStackTrace();
          createEncodingEvent("crypto.base64.encode", {
    ...
    if (encodeToStringIntIntInt) {
      encodeToStringIntIntInt.implementation = safeImplementation("encodings:Base64.encodeToString[byte[],int,int,int]", encodeToStringIntIntInt, function(original, input, offset, len, flags) {
        const result = original.call(this, input, offset, len, flags);
        if (input.length !== 0) {
          const stack = threadInstance.currentThread().getStackTrace();
    ...
    if (encodeToStringByteInt) {
      encodeToStringByteInt.implementation = safeImplementation("encodings:Base64.encodeToString[byte[],int]", encodeToStringByteInt, function(original, input, flags) {
        const result = original.call(this, input, flags);
        if (input.length !== 0) {
          const stack = threadInstance.currentThread().getStackTrace();
          createEncodingEvent("crypto.base64.encode_to_string", {
...
function charArrayToString(charArray) {
  if (charArray == null)
    return "(null)";
  if (!StringCls)
    return "(StringCls not initialized)";
  return StringCls.$new(charArray);
}
function hookKeystoreConstructor(KeyStore) {
  const ctor = safeOverload(KeyStore.$init, "keystore:KeyStore.$init", "java.security.KeyStoreSpi", "java.security.Provider", "java.lang.String");
  if (!ctor)
    return;
  ctor.implementation = safeImplementation("keystore:KeyStore.$init", ctor, function(original, keyStoreSpi, provider, type) {
...
function hookKeystoreGetInstance(KeyStore) {
  const getInstance = safeOverload(KeyStore.getInstance, "keystore:KeyStore.getInstance", "java.lang.String");
  if (!getInstance)
    return;
  getInstance.implementation = safeImplementation("keystore:KeyStore.getInstance[String]", getInstance, function(original, type) {
...
function hookKeystoreGetInstance_Provider(KeyStore) {
  const getInstance = safeOverload(KeyStore.getInstance, "keystore:KeyStore.getInstance", "java.lang.String", "java.lang.String");
  if (!getInstance)
    return;
  getInstance.implementation = safeImplementation("keystore:KeyStore.getInstance[String,String]", getInstance, function(original, type, provider) {
    createKeystoreEvent("crypto.keystore.get_instance", {
...
function hookKeystoreGetInstance_Provider2(KeyStore) {
  const getInstance = safeOverload(KeyStore.getInstance, "keystore:KeyStore.getInstance", "java.lang.String", "java.security.Provider");
  if (!getInstance)
    return;
  getInstance.implementation = safeImplementation("keystore:KeyStore.getInstance[String,Provider]", getInstance, function(original, type, provider) {
    createKeystoreEvent("crypto.keystore.get_instance", {
...
function hookKeystoreLoad(KeyStore, dump2) {
  const load = safeOverload(KeyStore.load, "keystore:KeyStore.load", "java.security.KeyStore$LoadStoreParameter");
  if (!load)
    return;
  load.implementation = safeImplementation("keystore:KeyStore.load[LoadStoreParameter]", load, function(original, param) {
...
function hookKeystoreLoadStream(KeyStore, dump2) {
  const loadStream = safeOverload(KeyStore.load, "keystore:KeyStore.load", "java.io.InputStream", "[C");
  if (!loadStream)
    return;
  loadStream.implementation = safeImplementation("keystore:KeyStore.load[InputStream,char[]]", loadStream, function(original, stream, charArray) {
    createKeystoreEvent("crypto.keystore.load", {
...
function hookKeystoreStore(KeyStore) {
  const store = safeOverload(KeyStore.store, "keystore:KeyStore.store", "java.security.KeyStore$LoadStoreParameter");
  if (!store)
    return;
  store.implementation = safeImplementation("keystore:KeyStore.store[LoadStoreParameter]", store, function(original, param) {
...
function hookKeystoreStoreStream(KeyStore) {
  const storeStream = safeOverload(KeyStore.store, "keystore:KeyStore.store", "java.io.OutputStream", "[C");
  if (!storeStream)
    return;
  storeStream.implementation = safeImplementation("keystore:KeyStore.store[OutputStream,char[]]", storeStream, function(original, stream, charArray) {
    createKeystoreEvent("crypto.keystore.store", {
...
function hookKeystoreGetKey(KeyStore) {
  const getKey = safeOverload(KeyStore.getKey, "keystore:KeyStore.getKey", "java.lang.String", "[C");
  if (!getKey)
    return;
  getKey.implementation = safeImplementation("keystore:KeyStore.getKey", getKey, function(original, alias, charArray) {
...
function hookKeystoreSetEntry(KeyStore) {
  const setEntry = safeOverload(KeyStore.setEntry, "keystore:KeyStore.setEntry", "java.lang.String", "java.security.KeyStore$Entry", "java.security.KeyStore$ProtectionParameter");
  if (!setEntry)
    return;
  setEntry.implementation = safeImplementation("keystore:KeyStore.setEntry", setEntry, function(original, alias, entry, protection) {
...
function hookKeystoreSetKeyEntry(KeyStore) {
  const setKeyEntry = safeOverload(KeyStore.setKeyEntry, "keystore:KeyStore.setKeyEntry", "java.lang.String", "java.security.Key", "[C", "[Ljava.security.cert.Certificate;");
  if (!setKeyEntry)
    return;
  setKeyEntry.implementation = safeImplementation("keystore:KeyStore.setKeyEntry[String,Key,char[],Certificate[]]", setKeyEntry, function(original, alias, key, charArray, certs) {
...
function hookKeystoreSetKeyEntry2(KeyStore) {
  const setKeyEntry2 = safeOverload(KeyStore.setKeyEntry, "keystore:KeyStore.setKeyEntry2", "java.lang.String", "[B", "[Ljava.security.cert.Certificate;");
  if (!setKeyEntry2)
    return;
  setKeyEntry2.implementation = safeImplementation("keystore:KeyStore.setKeyEntry[String,byte[],Certificate[]]", setKeyEntry2, function(original, alias, key, certs) {
    createKeystoreEvent("crypto.keystore.set_key_entry", {
...
function hookKeystoreGetCertificate(KeyStore) {
  const getCertificate = safeOverload(KeyStore.getCertificate, "keystore:KeyStore.getCertificate", "java.lang.String");
  if (!getCertificate)
    return;
  getCertificate.implementation = safeImplementation("keystore:KeyStore.getCertificate", getCertificate, function(original, alias) {
...
function hookKeystoreGetCertificateChain(KeyStore) {
  const getCertificateChain = safeOverload(KeyStore.getCertificateChain, "keystore:KeyStore.getCertificateChain", "java.lang.String");
  if (!getCertificateChain)
    return;
  getCertificateChain.implementation = safeImplementation("keystore:KeyStore.getCertificateChain", getCertificateChain, function(original, alias) {
...
function hookKeystoreGetEntry(KeyStore) {
  const getEntry = safeOverload(KeyStore.getEntry, "keystore:KeyStore.getEntry", "java.lang.String", "java.security.KeyStore$ProtectionParameter");
  if (!getEntry)
    return;
  getEntry.implementation = safeImplementation("keystore:KeyStore.getEntry", getEntry, function(original, alias, protection) {
...
function install_url_hooks() {
  ...
  safePerform("web:install_url_hooks", () => {
    ...
    if (URL) {
      ...
      if (urlInit) {
        urlInit.implementation = safeImplementation("web:URL.$init", urlInit, function(original, urlString) {
      ...
      if (openConnection) {
        openConnection.implementation = safeImplementation("web:URL.openConnection", openConnection, function(original) {
    ...
    if (HttpURLConnection) {
      ...
      connectRef.implementation = safeImplementation("web:HttpURLConnection.connect[url_hooks]", connectRef, function(original) {
    ...
    if (URI) {
      ...
      if (uriInit) {
        uriInit.implementation = safeImplementation("web:URI.$init", uriInit, function(original, uriString) {
...
function install_http_hooks() {
  ...
  safePerform("web:install_http_hooks", () => {
    const HttpURLConnection = safeUse("java.net.HttpURLConnection", "web:install_http_hooks");
    if (!HttpURLConnection)
      return;
    const setRequestMethodRef = HttpURLConnection.setRequestMethod;
    setRequestMethodRef.implementation = safeImplementation("web:HttpURLConnection.setRequestMethod", setRequestMethodRef, function(original, method) {
    ...
    const connectRef = HttpURLConnection.connect;
    connectRef.implementation = safeImplementation("web:HttpURLConnection.connect[http_hooks]", connectRef, function(original) {
      const result = original.call(this);
      try {
    ...
    getOutputStreamRef.implementation = safeImplementation("web:HttpURLConnection.getOutputStream", getOutputStreamRef, function(original) {
    ...
    getInputStreamRef.implementation = safeImplementation("web:HttpURLConnection.getInputStream", getInputStreamRef, function(original) {
...
function install_https_hooks() {
  ...
  safePerform("web:install_https_hooks", () => {
    const HttpsURLConnection = safeUse("javax.net.ssl.HttpsURLConnection", "web:install_https_hooks");
    if (!HttpsURLConnection)
      return;
    const setRequestMethodRef = HttpsURLConnection.setRequestMethod;
    setRequestMethodRef.implementation = safeImplementation("web:HttpsURLConnection.setRequestMethod", setRequestMethodRef, function(original, method) {
    ...
    connectRef.implementation = safeImplementation("web:HttpsURLConnection.connect", connectRef, function(original) {
      const result = original.call(this);
      try {
        const responseCode = this.getResponseCode();
    ...
    getInputStreamRef.implementation = safeImplementation("web:HttpsURLConnection.getInputStream", getInputStreamRef, function(original) {
      const inputStream = original.call(this);
...
function install_okhttp_hooks() {
  ...
  safePerform("web:install_okhttp_hooks", () => {
    ...
    if (OkHttpClient) {
      const newCall = safeOverload(OkHttpClient.newCall, "web:OkHttpClient.newCall", "okhttp3.Request");
      if (newCall) {
        newCall.implementation = safeImplementation("web:OkHttpClient.newCall", newCall, function(original, request) {
          const headers = {};
          try {
    ...
    if (OkHttpClientOld) {
      const newCallOld = safeOverload(OkHttpClientOld.newCall, "web:okhttp.OkHttpClient.newCall", "okhttp.Request");
      if (newCallOld) {
        newCallOld.implementation = safeImplementation("web:okhttp.OkHttpClient.newCall", newCallOld, function(original, request) {
    ...
    const HttpURLConnectionImpl = safeUse("com.android.okhttp.internal.huc.HttpURLConnectionImpl", "web:install_okhttp_hooks");
    if (HttpURLConnectionImpl) {
      const setRequestPropertyRef = HttpURLConnectionImpl.setRequestProperty;
      setRequestPropertyRef.implementation = safeImplementation("web:HttpURLConnectionImpl.setRequestProperty", setRequestPropertyRef, function(original, name, value) {
      ...
      setRequestMethodRef.implementation = safeImplementation("web:HttpURLConnectionImpl.setRequestMethod", setRequestMethodRef, function(original, method) {
...
function install_webview_hooks() {
  ...
  safePerform("web:install_webview_hooks", () => {
    ...
    if (WebView) {
      const loadUrlBasic = safeOverload(WebView.loadUrl, "web:WebView.loadUrl", "java.lang.String");
      if (loadUrlBasic) {
        loadUrlBasic.implementation = safeImplementation("web:WebView.loadUrl[String]", loadUrlBasic, function(original, url) {
      ...
      const loadUrlWithHeaders = safeOverload(WebView.loadUrl, "web:WebView.loadUrl", "java.lang.String", "java.util.Map");
      if (loadUrlWithHeaders) {
        loadUrlWithHeaders.implementation = safeImplementation("web:WebView.loadUrl[String,Map]", loadUrlWithHeaders, function(original, url, additionalHttpHeaders) {
      ...
      loadDataRef.implementation = safeImplementation("web:WebView.loadData", loadDataRef, function(original, data, mimeType, encoding) {
      ...
      if (postUrlRef) {
        postUrlRef.implementation = safeImplementation("web:WebView.postUrl", postUrlRef, function(original, url, postData) {
    ...
    if (WebViewClient) {
      ...
      onPageStartedRef.implementation = safeImplementation("web:WebViewClient.onPageStarted", onPageStartedRef, function(original, view, url, favicon) {
      ...
      onPageFinishedRef.implementation = safeImplementation("web:WebViewClient.onPageFinished", onPageFinishedRef, function(original, view, url) {
      ...
      const shouldOverride = safeOverload(WebViewClient.shouldOverrideUrlLoading, "web:WebViewClient.shouldOverrideUrlLoading", "android.webkit.WebView", "java.lang.String");
      if (shouldOverride) {
        shouldOverride.implementation = safeImplementation("web:WebViewClient.shouldOverrideUrlLoading", shouldOverride, function(original, view, url) {
...
function install_retrofit_hooks() {
  ...
  safePerform("web:install_retrofit_hooks", () => {
    ...
    if (OkHttpCall) {
      ...
      executeRef.implementation = safeImplementation("web:OkHttpCall.execute", executeRef, function(original) {
    ...
    if (Call) {
      ...
      enqueueRef.implementation = safeImplementation("web:Call.enqueue", enqueueRef, function(original, callback) {
        const request = this.request();
        if (request) {
...
function install_volley_hooks() {
  ...
  safePerform("web:install_volley_hooks", () => {
    const StringRequest = safeUse("com.android.volley.toolbox.StringRequest", "web:install_volley_hooks");
    if (StringRequest) {
      const volleyInit = safeOverload(StringRequest.$init, "web:StringRequest.$init", "int", "java.lang.String", "com.android.volley.Response$Listener", "com.android.volley.Response$ErrorListener");
      if (volleyInit) {
        volleyInit.implementation = safeImplementation("web:StringRequest.$init", volleyInit, function(original, method, url, listener, errorListener) {
    ...
    const RequestQueue = safeUse("com.android.volley.RequestQueue", "web:install_volley_hooks");
    if (RequestQueue) {
      const addRef = RequestQueue.add;
      addRef.implementation = safeImplementation("web:RequestQueue.add", addRef, function(original, request) {
        if (request.getUrl) {
...
function install_websocket_hooks() {
  ...
  safePerform("web:install_websocket_hooks", () => {
    ...
    if (WebSocket) {
      const sendText = safeOverload(WebSocket.send, "web:WebSocket.send", "java.lang.String");
      if (sendText) {
        sendText.implementation = safeImplementation("web:WebSocket.send[String]", sendText, function(original, text) {
    ...
    const WebSocketListener = safeUse("okhttp3.WebSocketListener", "web:install_websocket_hooks");
    if (WebSocketListener) {
      const onOpenRef = WebSocketListener.onOpen;
      onOpenRef.implementation = safeImplementation("web:WebSocketListener.onOpen", onOpenRef, function(original, webSocket, response) {
      ...
      const onMessageText = safeOverload(WebSocketListener.onMessage, "web:WebSocketListener.onMessage", "okhttp3.WebSocket", "java.lang.String");
      if (onMessageText) {
        onMessageText.implementation = safeImplementation("web:WebSocketListener.onMessage[String]", onMessageText, function(original, webSocket, text) {
...
function hook_java_socket_communication() {
  safePerform("sockets:hook_java_socket_communication", () => {
    const ServerSocket = safeUse("java.net.ServerSocket", "sockets:hook_java_socket_communication");
    const Socket2 = safeUse("java.net.Socket", "sockets:hook_java_socket_communication");
    const LocalServerSocket = safeUse("android.net.LocalServerSocket", "sockets:hook_java_socket_communication");
    const DatagramSocket = safeUse("java.net.DatagramSocket", "sockets:hook_java_socket_communication");
    if (ServerSocket) {
      const accept = safeOverload(ServerSocket.accept, "sockets:ServerSocket.accept");
      if (accept) {
        accept.implementation = safeImplementation("sockets:ServerSocket.accept", accept, function(original) {
    ...
    if (Socket2) {
      const socketInit = safeOverload(Socket2.$init, "sockets:Socket.$init", "java.lang.String", "int");
      if (socketInit) {
        socketInit.implementation = safeImplementation("sockets:Socket.$init", socketInit, function(original, host, port) {
      ...
      const connectWithTimeout = safeOverload(Socket2.connect, "sockets:Socket.connect", "java.net.SocketAddress", "int");
      if (connectWithTimeout) {
        connectWithTimeout.implementation = safeImplementation("sockets:Socket.connect[SocketAddress,int]", connectWithTimeout, function(original, p_endpoint, p_timeout) {
      ...
      const connectBasic = safeOverload(Socket2.connect, "sockets:Socket.connect", "java.net.SocketAddress");
      if (connectBasic) {
        connectBasic.implementation = safeImplementation("sockets:Socket.connect[SocketAddress]", connectBasic, function(original, p_endpoint) {
    ...
    if (LocalServerSocket) {
      const localAccept = safeOverload(LocalServerSocket.accept, "sockets:LocalServerSocket.accept");
      if (localAccept) {
        localAccept.implementation = safeImplementation("sockets:LocalServerSocket.accept", localAccept, function(original) {
          var result = original.call(this);
    ...
    if (DatagramSocket) {
      const datagramConnect = safeOverload(DatagramSocket.connect, "sockets:DatagramSocket.connect", "java.net.InetAddress", "int");
      if (datagramConnect) {
        datagramConnect.implementation = safeImplementation("sockets:DatagramSocket.connect", datagramConnect, function(original, address, port) {
...
function hook_java_process_creation() {
  safePerform("process:hook_java_process_creation", () => {
    const Process2 = safeUse("android.os.Process", "process:hook_java_process_creation");
    if (!Process2)
      return;
    ...
    const threadInstance = threadDef.$new();
    if (Process2.start) {
      const startRef = Process2.start;
      startRef.implementation = safeImplementation("process:Process.start", startRef, function(original, ...args) {
        const [processClass, niceName, uid, gid, gids, debugFlags, mountExternal, targetSdkVersion, seInfo, abi, instructionSet, appDataDir, zygoteArgs] = args;
        const stack = threadInstance.currentThread().getStackTrace();
    ...
    if (Process2.killProcess) {
      const killProcessRef = Process2.killProcess;
      killProcessRef.implementation = safeImplementation("process:Process.killProcess", killProcessRef, function(original, pid) {
    ...
    if (Process2.sendSignal) {
      const sendSignalRef = Process2.sendSignal;
      sendSignalRef.implementation = safeImplementation("process:Process.sendSignal", sendSignalRef, function(original, pid, signal) {
...
function hook_runtime() {
  safePerform("runtime:hook_runtime", () => {
    const Runtime2 = safeUse("java.lang.Runtime", "runtime:hook_runtime");
    if (!Runtime2)
      return;
    ...
    if (!threadDef)
      return;
    const threadInstance = threadDef.$new();
    Runtime2.exec.overloads.forEach((overload, index) => {
      overload.implementation = safeImplementation(`runtime:Runtime.exec[${index}]`, overload, function(original, ...args) {
        const stack = threadInstance.currentThread().getStackTrace();
        let commandStr = null;
    ...
    Runtime2.loadLibrary.overloads.forEach((overload, index) => {
      overload.implementation = safeImplementation(`runtime:Runtime.loadLibrary[${index}]`, overload, function(original, libname) {
        const stack = threadInstance.currentThread().getStackTrace();
    ...
    Runtime2.load.overloads.forEach((overload, index) => {
      overload.implementation = safeImplementation(`runtime:Runtime.load[${index}]`, overload, function(original, filename) {
        const stack = threadInstance.currentThread().getStackTrace();
...
function trace_reflection() {
  safePerform("runtime:trace_reflection", () => {
    ...
    if (classDef) {
      const getMethod = safeOverload(classDef.getMethod, "runtime:Class.getMethod", "java.lang.String", "[Ljava.lang.Class;");
      if (getMethod) {
        getMethod.implementation = safeImplementation("runtime:Class.getMethod", getMethod, function(original, methodName, paramTypes) {
      ...
      const getDeclaredMethod = safeOverload(classDef.getDeclaredMethod, "runtime:Class.getDeclaredMethod", "java.lang.String", "[Ljava.lang.Class;");
      if (getDeclaredMethod) {
        getDeclaredMethod.implementation = safeImplementation("runtime:Class.getDeclaredMethod", getDeclaredMethod, function(original, methodName, paramTypes) {
          const method = original.call(this, methodName, paramTypes);
          const stack = threadInstance.currentThread().getStackTrace();
      ...
      const forName = safeOverload(classDef.forName, "runtime:Class.forName", "java.lang.String", "boolean", "java.lang.ClassLoader");
      if (forName) {
        forName.implementation = safeImplementation("runtime:Class.forName", forName, function(original, class_name, flag, class_loader) {
    ...
    if (classLoaderDef) {
      const loadClass = safeOverload(classLoaderDef.loadClass, "runtime:ClassLoader.loadClass", "java.lang.String", "boolean");
      if (loadClass) {
        loadClass.implementation = safeImplementation("runtime:ClassLoader.loadClass", loadClass, function(original, class_name, resolve) {
          let isInternal = false;
          for (const internalClass of internalClasses) {
            if (class_name.startsWith(internalClass)) {
              isInternal = true;
    ...
    if (Method) {
      const invoke = safeOverload(Method.invoke, "runtime:Method.invoke", "java.lang.Object", "[Ljava.lang.Object;");
      if (invoke) {
        invoke.implementation = safeImplementation("runtime:Method.invoke", invoke, function(original, instance, args) {
          const stack = threadInstance.currentThread().getStackTrace();
...
function hook_bluetooth() {
  safePerform("bluetooth:hook_bluetooth", () => {
    const BluetoothGatt = safeUse("android.bluetooth.BluetoothGatt", "bluetooth:hook_bluetooth");
    const BluetoothGattCharacteristic = safeUse("android.bluetooth.BluetoothGattCharacteristic", "bluetooth:hook_bluetooth");
    const BluetoothAdapter = safeUse("android.bluetooth.BluetoothAdapter", "bluetooth:hook_bluetooth");
    const BluetoothDevice = safeUse("android.bluetooth.BluetoothDevice", "bluetooth:hook_bluetooth");
    const threadDef = safeUse("java.lang.Thread", "bluetooth:hook_bluetooth");
    if (!threadDef)
      return;
    const threadInstance = threadDef.$new();
    if (BluetoothGatt) {
      const readChar = safeOverload(BluetoothGatt.readCharacteristic, "bluetooth:BluetoothGatt.readCharacteristic", "android.bluetooth.BluetoothGattCharacteristic");
      if (readChar) {
        readChar.implementation = safeImplementation("bluetooth:BluetoothGatt.readCharacteristic", readChar, function(original, characteristic) {
          const stack = threadInstance.currentThread().getStackTrace();
    ...
    if (BluetoothGattCharacteristic) {
      const setValue = safeOverload(BluetoothGattCharacteristic.setValue, "bluetooth:BluetoothGattCharacteristic.setValue", "[B");
      if (setValue) {
        setValue.implementation = safeImplementation("bluetooth:BluetoothGattCharacteristic.setValue", setValue, function(original, value) {
          const stack = threadInstance.currentThread().getStackTrace();
    ...
    if (BluetoothAdapter) {
      if (BluetoothAdapter.getDefaultAdapter) {
        ...
        getDefaultAdapterRef.implementation = safeImplementation("bluetooth:BluetoothAdapter.getDefaultAdapter", getDefaultAdapterRef, function(original) {
          const stack = threadInstance.currentThread().getStackTrace();
          const result = original.call(this);
      ...
      enableRef.implementation = safeImplementation("bluetooth:BluetoothAdapter.enable", enableRef, function(original) {
        const stack = threadInstance.currentThread().getStackTrace();
        const result = original.call(this);
      ...
      disableRef.implementation = safeImplementation("bluetooth:BluetoothAdapter.disable", disableRef, function(original) {
        const stack = threadInstance.currentThread().getStackTrace();
        const result = original.call(this);
      ...
      startDiscoveryRef.implementation = safeImplementation("bluetooth:BluetoothAdapter.startDiscovery", startDiscoveryRef, function(original) {
        const stack = threadInstance.currentThread().getStackTrace();
        const result = original.call(this);
    ...
    if (BluetoothDevice) {
      ...
      createBondRef.implementation = safeImplementation("bluetooth:BluetoothDevice.createBond", createBondRef, function(original) {
        const stack = threadInstance.currentThread().getStackTrace();
...
function hook_location() {
  safePerform("location:hook_location", () => {
    ...
    const threadInstance = threadDef.$new();
    if (LocationManager) {
      const getLastKnown = safeOverload(LocationManager.getLastKnownLocation, "location:LocationManager.getLastKnownLocation", "java.lang.String");
      if (getLastKnown) {
        getLastKnown.implementation = safeImplementation("location:LocationManager.getLastKnownLocation", getLastKnown, function(original, provider) {
      ...
      const requestUpdatesBasic = safeOverload(LocationManager.requestLocationUpdates, "location:LocationManager.requestLocationUpdates", "java.lang.String", "long", "float", "android.location.LocationListener");
      if (requestUpdatesBasic) {
        requestUpdatesBasic.implementation = safeImplementation("location:LocationManager.requestLocationUpdates[basic]", requestUpdatesBasic, function(original, provider, minTime, minDistance, listener) {
          const stack = threadInstance.currentThread().getStackTrace();
      ...
      const requestUpdatesLooper = safeOverload(LocationManager.requestLocationUpdates, "location:LocationManager.requestLocationUpdates", "java.lang.String", "long", "float", "android.location.LocationListener", "android.os.Looper");
      if (requestUpdatesLooper) {
        requestUpdatesLooper.implementation = safeImplementation("location:LocationManager.requestLocationUpdates[with_looper]", requestUpdatesLooper, function(original, provider, minTime, minDistance, listener, looper) {
          const stack = threadInstance.currentThread().getStackTrace();
          createLocationEvent("location.request_updates", {
            library: "android.location.LocationManager",
            method: "requestLocationUpdates",
    ...
    if (Location) {
      ...
      getLatitudeRef.implementation = safeImplementation("location:Location.getLatitude", getLatitudeRef, function(original) {
      ...
      getLongitudeRef.implementation = safeImplementation("location:Location.getLongitude", getLongitudeRef, function(original) {
...
function hook_playstore_location_api() {
  safePerform("location:hook_playstore_location_api", () => {
    const FusedLocationProviderClient = safeUse("com.google.android.gms.location.FusedLocationProviderClient", "location:hook_playstore_location_api");
    ...
    if (!threadDef)
      return;
    const threadInstance = threadDef.$new();
    const getLastLocation = safeOverload(FusedLocationProviderClient.getLastLocation, "location:FusedLocationProviderClient.getLastLocation");
    if (getLastLocation) {
      getLastLocation.implementation = safeImplementation("location:FusedLocationProviderClient.getLastLocation", getLastLocation, function(original) {
        const stack = threadInstance.currentThread().getStackTrace();
        const result = original.call(this);
...
function hook_sms() {
  safePerform("telephony:hook_sms", () => {
    ...
    if (!threadDef)
      return;
    const threadInstance = threadDef.$new();
    const sendText = safeOverload(SmsManager.sendTextMessage, "telephony:SmsManager.sendTextMessage", "java.lang.String", "java.lang.String", "java.lang.String", "android.app.PendingIntent", "android.app.PendingIntent");
    if (sendText) {
      sendText.implementation = safeImplementation("telephony:SmsManager.sendTextMessage", sendText, function(original, destinationAddress, scAddress, text, sentIntent, deliveryIntent) {
        const stack = threadInstance.currentThread().getStackTrace();
    ...
    const sendMultipart = safeOverload(SmsManager.sendMultipartTextMessage, "telephony:SmsManager.sendMultipartTextMessage", "java.lang.String", "java.lang.String", "java.util.ArrayList", "java.util.ArrayList", "java.util.ArrayList");
    if (sendMultipart) {
      sendMultipart.implementation = safeImplementation("telephony:SmsManager.sendMultipartTextMessage", sendMultipart, function(original, destinationAddress, scAddress, parts, sentIntents, deliveryIntents) {
        const stack = threadInstance.currentThread().getStackTrace();
...
function hook_device_infos() {
  safePerform("telephony:hook_device_infos", () => {
    ...
    const seenEvents = {};
    if (systemProperties) {
      const sysPropsGet = safeOverload(systemProperties.get, "telephony:SystemProperties.get", "java.lang.String");
      if (sysPropsGet) {
        sysPropsGet.implementation = safeImplementation("telephony:SystemProperties.get", sysPropsGet, function(original, key_value) {
    ...
    if (telephonyManager) {
      const getLine1Number = safeOverload(telephonyManager.getLine1Number, "telephony:TelephonyManager.getLine1Number");
      if (getLine1Number) {
        getLine1Number.implementation = safeImplementation("telephony:TelephonyManager.getLine1Number", getLine1Number, function(original) {
          const result = original.call(this);
          const stack = threadInstance.currentThread().getStackTrace();
      ...
      const getSubscriberId = safeOverload(telephonyManager.getSubscriberId, "telephony:TelephonyManager.getSubscriberId");
      if (getSubscriberId) {
        getSubscriberId.implementation = safeImplementation("telephony:TelephonyManager.getSubscriberId", getSubscriberId, function(original) {
          const result = original.call(this);
          const stack = threadInstance.currentThread().getStackTrace();
      ...
      const getDeviceId = safeOverload(telephonyManager.getDeviceId, "telephony:TelephonyManager.getDeviceId");
      if (getDeviceId) {
        getDeviceId.implementation = safeImplementation("telephony:TelephonyManager.getDeviceId", getDeviceId, function(original) {
          const result = original.call(this);
          const stack = threadInstance.currentThread().getStackTrace();
      ...
      const getImei = safeOverload(telephonyManager.getImei, "telephony:TelephonyManager.getImei");
      if (getImei) {
        getImei.implementation = safeImplementation("telephony:TelephonyManager.getImei", getImei, function(original) {
          const result = original.call(this);
          const stack = threadInstance.currentThread().getStackTrace();
      ...
      const getSimOperator = safeOverload(telephonyManager.getSimOperator, "telephony:TelephonyManager.getSimOperator");
      if (getSimOperator) {
        getSimOperator.implementation = safeImplementation("telephony:TelephonyManager.getSimOperator", getSimOperator, function(original) {
          const result = original.call(this);
          const stack = threadInstance.currentThread().getStackTrace();
    ...
    if (bluetoothAdapter) {
      ...
      getAddressRef.implementation = safeImplementation("telephony:BluetoothAdapter.getAddress", getAddressRef, function(original) {
        const result = original.call(this);
        const stack = threadInstance.currentThread().getStackTrace();
    ...
    if (wifiInfo) {
      ...
      getMacRef.implementation = safeImplementation("telephony:WifiInfo.getMacAddress", getMacRef, function(original) {
        const result = original.call(this);
        const stack = threadInstance.currentThread().getStackTrace();
      ...
      getSSIDRef.implementation = safeImplementation("telephony:WifiInfo.getSSID", getSSIDRef, function(original) {
        const result = original.call(this);
        const stack = threadInstance.currentThread().getStackTrace();
      ...
      getBSSIDRef.implementation = safeImplementation("telephony:WifiInfo.getBSSID", getBSSIDRef, function(original) {
        const result = original.call(this);
        const stack = threadInstance.currentThread().getStackTrace();
    ...
    if (contentResolver) {
      const query1 = safeOverload(contentResolver.query, "telephony:ContentResolver.query", "android.net.Uri", "[Ljava.lang.String;", "android.os.Bundle", "android.os.CancellationSignal");
      if (query1) {
        query1.implementation = safeImplementation("telephony:ContentResolver.query[Uri,String[],Bundle,CancellationSignal]", query1, function(original, uri, str, bundle, sig) {
          const stack = threadInstance.currentThread().getStackTrace();
      ...
      const query2 = safeOverload(contentResolver.query, "telephony:ContentResolver.query", "android.net.Uri", "[Ljava.lang.String;", "java.lang.String", "[Ljava.lang.String;", "java.lang.String");
      if (query2) {
        query2.implementation = safeImplementation("telephony:ContentResolver.query[Uri,String[],String,String[],String]", query2, function(original, uri, astr, bstr, cstr, dstr) {
          const stack = threadInstance.currentThread().getStackTrace();
          if (uri == "content://com.google.android.gsf.gservicesa") {
            createTelephonyEvent("telephony.content_resolver.query_gsf", {
              library: "android.content.ContentResolver",
      ...
      const query3 = safeOverload(contentResolver.query, "telephony:ContentResolver.query", "android.net.Uri", "[Ljava.lang.String;", "java.lang.String", "[Ljava.lang.String;", "java.lang.String", "android.os.CancellationSignal");
      if (query3) {
        query3.implementation = safeImplementation("telephony:ContentResolver.query[Uri,String[],String,String[],String,CancellationSignal]", query3, function(original, uri, astr, bstr, cstr, sig) {
          const stack = threadInstance.currentThread().getStackTrace();
          if (uri == "content://com.google.android.gsf.gservicesa") {
            createTelephonyEvent("telephony.content_resolver.query_gsf", {
              library: "android.content.ContentResolver",
    ...
    if (secureSettings) {
      ...
      getStringRef.implementation = safeImplementation("telephony:SettingsSecure.getString", getStringRef, function(original, contentresolver, query) {
...
var PROFILE_HOOKING_TYPE22 = "BYPASS_DETECTION";
function createBypassEvent(eventType, data) {
  const event = {
    event_type: eventType,
    timestamp: Date.now(),
    ...data
...
function install_root_detection_bypass() {
  ...
  Java.perform(() => {
    try {
      ...
      Runtime2.exec.overload("java.lang.String").implementation = function(command) {
      ...
      Runtime2.exec.overload("[Ljava.lang.String;").implementation = function(commands) {
...
function install_frida_detection_bypass() {
  devlog("Installing Frida detection bypass hooks");
  Java.perform(() => {
    try {
      const File2 = Java.use("java.io.File");
      const originalExists = File2.exists;
      File2.exists.implementation = function() {
        const path = this.getAbsolutePath();
      ...
      const Socket2 = Java.use("java.net.Socket");
      ...
      ActivityManager.getRunningAppProcesses.implementation = function() {
        ...
        if (processes) {
          ...
          for (let i = processArray.size() - 1; i >= 0; i--) {
            ...
            if (processName.includes("frida") || processName.includes("gum") || processName.includes("gmain") || processName.includes("pool-frida")) {
      ...
      Thread2.getName.implementation = function() {
...
function install_debugger_detection_bypass() {
...
function install_emulator_detection_bypass() {
...
function install_hook_detection_bypass() {
    ...
    try {
      ...
      throwable.getStackTrace.implementation = function() {
        ...
        for (let i = 0; i < stack.length; i++) {
          ...
          if (!className.includes("de.robv.android.xposed") && !className.includes("com.android.internal.os.ZygoteInit") && !className.includes("frida") && !className.includes("gum")) {
