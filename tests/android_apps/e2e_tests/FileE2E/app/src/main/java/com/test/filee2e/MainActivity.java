...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        boolean deactivateUnlinkMode = getIntent().getBooleanExtra(
            "file_e2e_deactivate_unlink",
            false
        );
        ...
        try {
            ...
            if (deactivateUnlinkMode) {
                try {
                    new FileDeleteTests(this).runBlockedUnlinkTest();
                    Log.i(TAG, "FileDeleteTests blocked unlink completed");
                } catch (Throwable t) {
                    Log.e(TAG, "FileDeleteTests blocked unlink failed", t);
                }
            } else {
