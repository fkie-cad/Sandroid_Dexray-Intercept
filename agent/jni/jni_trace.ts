...
function initJniConfig(): Config {
    ...
    builder.libraries = ["*"]; 
    builder.backtrace = "none";
    builder.includeExports = [];
    builder.excludeExports = [];
    builder.env = true;
    builder.vm = true;
...
const jniEnvCallback: JNIInvocationCallback = {
    ...
    onLeave (retval: JNINativeReturnValue): void {
        ...
        createJniEvent("jni.env.call", {
            jni_method: methodName,
            jni_args: (this.args as any[]).map(String),
            jni_ret: String(retval.get()),
...
const javaVMCallback: JNIInvocationCallback = {
    ...
    onLeave (retval: JNINativeReturnValue): void {
        ...
        createJniEvent("jni.vm.call", {
            jvm_method: methodName,
            jvm_args: (this.args as any[]).map(String),
            jvm_ret: String(retval.get()),
            thread_id: this.threadId
        });
    }
};
