📦
502358 /agent/anticertpinning/config_custom.js
307721 /agent/anticertpinning/config_custom.js.map
✄
...
// agent/anticertpinning/native-tls-hook.js
function nativeTLSHook() {
  const TARGET_LIBS = [
...
[ACP] Could not load ${targetLib.name} to hook TLS`);
    }
  });
}
function patchTargetLib(targetModule, targetName) {
  ...
  const buildVerificationCallback = (realCallbackAddr) => {
    if (!verificationCallbackCache[realCallbackAddr]) {
      ...
      let pendingCheckThreads = /* @__PURE__ */ new Set();
  ...
  if (customVerifyAddrs.length) {
    if (DEBUG_MODE) {
      console.log(`[+] ACP: Patched ${customVerifyAddrs.length} ${targetName} verification methods`);
    }
    if (DEBUG_MODE) console.log(`[*] ACP: Hooked native TLS lib ${targetName}`);
  } else {
    if (DEBUG_MODE) console.log(`
[*] ACP: Hooking native TLS lib ${targetName} failed - no verification methods found`);
  }
  ...
  if (get_psk_identity_addr) {
    ...
    if (DEBUG_MODE) console.log(`[*] ACP: Patched ${customVerifyAddrs.length} custom_verify methods, but couldn't find get_psk_identity`);
  }
}

// agent/anticertpinning/native-connect-hook.js
function nativeConnectHook() {
  (() => {
    ...
    try {
      ...
      send2 = new NativeFunction(systemModule.getExportByName("send"), "ssize_t", ["int", "pointer", "size_t", "int"]);
      ...
      if (DEBUG_MODE) console.log(`[!] ACP: Failed to set up native hooks: ${e.message}`);
      if (DEBUG_MODE) console.log("[!] ACP: Could not initialize system functions to to hook raw traffic");
      return;
    }
      ...
      onEnter(args) {
        ...
        if (isTCP || isUDP) {
          ...
          if (shouldBeBlocked) {
            ...
            if (DEBUG_MODE) console.log(`[*] ACP: Blocking QUIC connection to ${getReadableAddress(hostBytes, isIPv6)}:${port}`);
            this.state = "Blocked";
            ...
            if (DEBUG_MODE) console.log(`[*] ACP: Manually intercepting ${sockType} connection to ${getReadableAddress(hostBytes, isIPv6)}:${port}`);
            ...
            if (DEBUG_MODE) {
              console.log(`[*] ACP: Allowing unintercepted ${sockType} connection to port ${port}`);
            }
            this.state = "ignored";
          }
        } else {
          if (DEBUG_MODE) console.log(`[*] ACP: Ignoring ${sockType} connection`);
          this.state = "ignored";
        }
      },
      onLeave: function(retval) {
        ...
        if (this.state === "intercepting" && PROXY_SUPPORTS_SOCKS5) {
          ...
          if (connectSuccess) {
            ...
            if (DEBUG_MODE) console.log(`[*] ACP: SOCKS: Failed to connect to proxy at ${PROXY_HOST}:${PROXY_PORT}`);
          }
          if (this.isNonBlocking) {
          ...
          if (handshakeSuccess) {
            ...
            if (DEBUG_MODE) console.log(`[*] ACP: SOCKS redirect successful for fd ${this.sockFd} to ${readableHost}:${port}`);
            retval.replace(0);
          } else {
            if (DEBUG_MODE) console.log(`[*] ACP: SOCKS redirect FAILED for fd ${this.sockFd}`);
            retval.replace(-1);
          }
        } else if (DEBUG_MODE) {
          ...
          console.log(
            `[*] ACP: ${this.state} ${sockType} fd ${fd} to ${JSON.stringify(address)} (${retval.toInt32()})`
          );
        }
      }
    });
    if (DEBUG_MODE) console.log(`[*] ACP: Redirecting ${IGNORED_NON_HTTP_PORTS.length === 0 ? "all" : "all unrecognized"} TCP connections to ${PROXY_HOST}:${PROXY_PORT}`);
    ...
    function performSocksHandshake(sockfd, targetHostBytes, targetPort, isIPv6) {
      ...
      if (send2(sockfd, hello, 3, 0) < 0) {
        if (DEBUG_MODE) console.log("[*] ACP: SOCKS: Failed to send hello");
        return false;
      }
      const response = Memory.alloc(2);
      if (recv(sockfd, response, 2, 0) < 0) {
        if (DEBUG_MODE) console.log("[*] ACP: SOCKS: Failed to receive server choice");
        return false;
      }
      if (response.readU8() !== 5 || response.add(1).readU8() !== 0) {
        if (DEBUG_MODE) console.log("[*] ACP: SOCKS: Server rejected auth method");
        return false;
      }
      let req = [5, 1, 0];
      if (isIPv6) {
      ...
      if (send2(sockfd, reqBuf, req.length, 0) < 0) {
        if (DEBUG_MODE) console.log("[*] ACP: SOCKS: Failed to send connection request");
        return false;
      }
      const replyHeader = Memory.alloc(4);
      if (recv(sockfd, replyHeader, 4, 0) < 0) {
        if (DEBUG_MODE) console.log("[*] ACP: SOCKS: Failed to receive reply header");
        return false;
      }
      const replyCode = replyHeader.add(1).readU8();
      if (replyCode !== 0) {
        if (DEBUG_MODE) console.log(`[ACP] SOCKS: Server returned error code ${replyCode}`);
        return false;
      }
...
// agent/anticertpinning/android-system-certificate-injection.js
function androidSystemCertInjection() {
  frida_java_bridge_default.perform(() => {
    ...
    const TrustManagerImpl = frida_java_bridge_default.use("com.android.org.conscrypt.TrustManagerImpl");
    TrustManagerImpl.checkTrustedRecursive.implementation = function(x509certs, n0, n1, host, b, al1, al2, s) {
      return al1;
    };
...
// agent/anticertpinning/android-proxy-override.js
function androidProxyOverride() {
  frida_java_bridge_default.perform(() => {
    ...
    frida_java_bridge_default.use("java.lang.System").clearProperty.implementation = function(property) {
      if (controlledSystemProperties.includes(property)) {
        if (DEBUG_MODE) console.log(`[*] ACP: Ignoring attempt to clear ${property} system property`);
    ...
    frida_java_bridge_default.use("java.lang.System").setProperty.implementation = function(property) {
      if (controlledSystemProperties.includes(property)) {
        if (DEBUG_MODE) console.log(`[*] ACP: Ignoring attempt to override ${property} system property`);
        return this.getProperty(property);
      }
    ...
    if (DEBUG_MODE) console.log(`[*] ACP: Proxy system configuration overridden to ${PROXY_HOST}:${PROXY_PORT}`);
    ...
    const proxySelectorClasses = frida_java_bridge_default.enumerateMethods("*!select(java.net.URI): java.util.List/s").flatMap(
      (matchingLoader) => matchingLoader.classes.map((classData) => frida_java_bridge_default.use(classData.name)).filter((Cls) => ProxySelector.class.isAssignableFrom(Cls.class))
    );
    proxySelectorClasses.forEach((ProxySelectorCls) => {
      if (DEBUG_MODE) {
        console.log(`[*] ACP: Rewriting ${ProxySelectorCls.toString()}`);
      }
      ProxySelectorCls.select.implementation = () => getTargetProxyList();
    });
    if (DEBUG_MODE) console.log(`[*] ACP: Proxy configuration overridden to ${PROXY_HOST}:${PROXY_PORT}`);
  });
}

// agent/anticertpinning/android-disable-root-detection.js
function androidDisableRootDetection() {
  (() => {
    let loggedRootDetectionWarning = false;
    function logFirstRootDetection() {
      if (!loggedRootDetectionWarning) {
        if (DEBUG_MODE) console.log("[*] ACP: => Blocked possible root detection checks. Enable DEBUG_MODE for more details.");
          ...
          onLeave(retval) {
            if (retval.toInt32() !== 0) {
              ...
              if (ROOT_INDICATORS.paths.has(this.path) || path.includes("magisk") || path.includes("/su") || path.endsWith("/su")) {
                if (DEBUG_MODE) {
                  console.log(`[*] ACP: Blocked possible root-detection: fopen ${this.path}`);
        ...
        Interceptor.attach(access, {
          ...
          onLeave(retval) {
            if (retval.toInt32() === 0) {
              ...
              if (ROOT_INDICATORS.paths.has(this.path) || path.includes("magisk") || path.includes("/su") || path.endsWith("/su")) {
                if (DEBUG_MODE) {
                  console.debug(`[*] ACP: Blocked possible root detection: access ${this.path}`);
                } else logFirstRootDetection();
      ...
      if (stat) {
        Interceptor.attach(stat, {
          ...
          onLeave(retval) {
            ...
            if (ROOT_INDICATORS.paths.has(this.path) || path.includes("magisk") || path.includes("/su") || path.endsWith("/su")) {
              if (DEBUG_MODE) {
                console.debug(`[*] ACP: Blocked possible root detection: stat ${this.path}`);
              } else logFirstRootDetection();
              retval.replace(ptr(-1));
            }
          }
      ...
      if (lstat) {
        Interceptor.attach(lstat, {
          ...
          onLeave(retval) {
            ...
            if (ROOT_INDICATORS.paths.has(this.path) || path.includes("magisk") || path.includes("/su") || path.endsWith("/su")) {
              if (DEBUG_MODE) {
                console.debug(`[*] ACP: Blocked possible root detection: lstat ${this.path}`);
              } else logFirstRootDetection();
              retval.replace(ptr(-1));
            }
          }
    ...
    function bypassJavaFileCheck() {
      ...
      UnixFileSystem.checkAccess.implementation = function(file, access) {
        if (isRootIndicatorFile(file)) {
          if (DEBUG_MODE) {
            console.debug(`[*] ACP: Blocked possible root detection: filesystem access check for ${file.getAbsolutePath()}`);
          } else logFirstRootDetection();
          return false;
        }
      ...
      File2.exists.implementation = function() {
        if (isRootIndicatorFile(this)) {
          if (DEBUG_MODE) {
            console.debug(`[*] ACP: Blocked possible root detection: file exists check for ${this.getAbsolutePath()}`);
          } else logFirstRootDetection();
          return false;
        }
        return this.exists();
      ...
      File2.length.implementation = function() {
        if (isRootIndicatorFile(this)) {
          if (DEBUG_MODE) {
            console.debug(`[*] ACP: Blocked possible root detection: file length check for ${this.getAbsolutePath()}`);
          } else logFirstRootDetection();
          return 0;
        }
        return this.length();
      ...
      FileInputStream.$init.overload("java.io.File").implementation = function(file) {
        if (isRootIndicatorFile(file)) {
          if (DEBUG_MODE) {
            console.debug(`[*] ACP: Blocked possible root detection: file stream for ${file.getAbsolutePath()}`);
          } else logFirstRootDetection();
    ...
    function setProp() {
      ...
      if (fingerprintMatch) {
        ...
        if (DEBUG_MODE) console.log(`[*] ACP: Unexpected BUILD fingerprint format: ${realFingerprint}`);
        ...
        Interceptor.attach(system_property_get, {
          ...
          onLeave(retval) {
            ...
            if (secureValue !== void 0) {
              if (DEBUG_MODE) {
                console.debug(`[*] ACP: Blocked possible root detection: system_property_get ${this.key}`);
              } else logFirstRootDetection();
      ...
      Runtime2.exec.overload("java.lang.String").implementation = function(cmd) {
        if (cmd.startsWith("getprop ")) {
          ...
          if (CONFIG.secureProps[prop]) {
            if (DEBUG_MODE) {
              console.debug(`[*] ACP: Blocked possible root detection: getprop ${prop}`);
            } else logFirstRootDetection();
            return null;
          }
        }
    ...
    function bypassRootPackageCheck() {
      ...
      ApplicationPackageManager.getPackageInfo.overload("java.lang.String", "int").implementation = function(str, i) {
        if (ROOT_INDICATORS.packages.has(str)) {
          if (DEBUG_MODE) {
            console.debug(`[*] ACP: Blocked possible root detection: package info for ${str}`);
          } else logFirstRootDetection();
      ...
      ProcessBuilder.command.overload("java.util.List").implementation = function(commands) {
        ...
        if (cmdArray.length > 0) {
          ...
          if (ROOT_INDICATORS.commands.has(cmd) || cmdArray.length > 1 && ROOT_INDICATORS.binaries.has(cmdArray[1].toString())) {
            if (DEBUG_MODE) {
              console.debug(`[*] ACP: Blocked possible root detection: ProcessBuilder with ${cmdArray.join(" ")}`);
            } else logFirstRootDetection();
      ...
      Runtime2.exec.overload("[Ljava.lang.String;").implementation = function(cmdArray) {
        if (cmdArray.length > 0) {
          ...
          if (ROOT_INDICATORS.commands.has(cmd) || cmdArray.length > 1 && ROOT_INDICATORS.binaries.has(cmdArray[1])) {
            if (DEBUG_MODE) {
              console.debug(`[*] ACP: Blocked possible root detection: Runtime.exec for ${cmdArray.join(" ")}`);
            } else logFirstRootDetection();
            return this.exec([""]);
          }
        }
      ...
      ProcessImpl.start.implementation = function(cmdArray, env, dir, redirects, redirectErrorStream) {
        if (cmdArray.length > 0) {
          ...
          if (ROOT_INDICATORS.commands.has(cmd) || ROOT_INDICATORS.binaries.has(arg)) {
            if (DEBUG_MODE) {
              console.debug(`[*] ACP: Blocked possible root detection: ProcessImpl.start for ${cmdArray.join(" ")}`);
            } else logFirstRootDetection();
    ...
    try {
      ...
      bypassShellCommands();
      if (DEBUG_MODE) console.log("[*] ACP: Disabled Android root detection");
    } catch (error) {
      if (DEBUG_MODE) console.log(`
[!] ACP: Error setting up root detection bypass: ${error}`);
    }
  })();
}

// agent/anticertpinning/android-certificate-unpinning.js
...
function androidCertUnpinning() {
  ...
  const getJavaClassIfExists = (clsName) => {
    try {
      return frida_java_bridge_default.use(clsName);
    } catch {
      return void 0;
    }
  };
  frida_java_bridge_default.perform(function() {
    if (DEBUG_MODE) console.log("\n[*] ACP: Disabling all recognized unpinning libraries");
    ...
    classesToPatch.forEach((targetClassName) => {
      ...
      if (!TargetClass) {
        if (DEBUG_MODE) console.log(`[*] ACP: [ ] ${targetClassName} *`);
        return;
      }
      ...
      patches.forEach(({ methodName, getMethod, overload, replacement }) => {
        ...
        if (targetMethodImplementations.length === 0) {
          if (DEBUG_MODE) console.log(`[*] ACP: [ ] ${targetClassName} ${methodDescription}`);
          return;
        }
          ...
          try {
            ...
            if (DEBUG_MODE) {
              targetMethod.implementation = function() {
                console.log(`[*] ACP: => ${patchName}`);
                return newImplementation.apply(this, arguments);
              };
            } else {
              targetMethod.implementation = newImplementation;
            }
            if (DEBUG_MODE) console.log(`[+] ACP: ${patchName}`);
            patchApplied = true;
          } catch (e) {
            if (DEBUG_MODE) console.log(`[!] ACP: ERROR: ${patchName} failed: ${e}`);
          }
        });
      });
      if (!patchApplied) {
        if (DEBUG_MODE) console.log(`[!] ACP: Matched class ${targetClassName} but could not patch any methods`);
      }
    });
    if (DEBUG_MODE) console.log("[*] ACP: Certificate unpinning completed");
  });
}

// agent/anticertpinning/android-certificate-unpinning-fallback.js
function androidCertUnpinningFallback() {
  ...
  const getFridaValues = (cls, values) => values.map(
  ...
  frida_java_bridge_default.perform(() => {
    try {
      ...
      const buildUnhandledErrorPatcher = (errorClassName, originalConstructor) => {
        return (errorArg) => {
          try {
            ...
            if (DEBUG_MODE) console.log(`[!] ACP: ${errorTypeName}: ${errorMessage}`);
            if (DEBUG_MODE) console.log(`[!] ACP:: Thrown by ${className}->${methodName}`);
            ...
            callingMethod.overloads.forEach((failingMethod) => {
              if (failingMethod.implementation) {
                if (DEBUG_MODE) console.log("[!] ACP: Already patched - but still failing!");
                return;
              }
              if (isOkHttpCheckMethod(errorMessage, failingMethod)) {
                failingMethod.implementation = () => {
                  if (DEBUG_MODE) console.log(`[*] ACP: => Fallback OkHttp patch`);
                };
                if (DEBUG_MODE) console.log(`[+] ACP: ${className}->${methodName} (fallback OkHttp patch)`);
                ...
                failingMethod.implementation = (chain) => {
                  if (DEBUG_MODE) console.log(`[*] ACP: => Fallback Appmattus+OkHttp patch`);
                  const proceed = chain[okHttpChain.proceedMethodName].bind(chain);
                  const request = chain[okHttpChain.requestFieldName].value;
                  return proceed(request);
                };
                if (DEBUG_MODE) console.log(`[+] ACP: ${className}->${methodName} (fallback Appmattus+OkHttp patch)`);
                ...
                if (argumentTypes.length === 2 && argumentTypes.every((t, i) => t === BASE_METHOD_ARGUMENTS[i]) && returnType === "void") {
                  failingMethod.implementation = (certs, authType) => {
                    if (DEBUG_MODE) console.log(`[*] ACP: => Fallback X509TrustManager patch of ${className} base method`);
                    const defaultTrustManager2 = getCustomX509TrustManager();
                    defaultTrustManager2.checkServerTrusted(certs, authType);
                  };
                  if (DEBUG_MODE) console.log(`[+] ACP: ${className}->${methodName} (fallback X509TrustManager base patch)`);
                  ...
                  failingMethod.implementation = function(certs, authType, _hostname) {
                    if (DEBUG_MODE) console.log(`[*] ACP: => Fallback X509TrustManager patch of ${className} extended method`);
                    try {
                      defaultTrustManager.checkServerTrusted(certs, authType);
                    } catch (e) {
                      if (DEBUG_MODE) console.log(`[!] ACP: Default TM threw: ${e}`);
                    }
                    return frida_java_bridge_default.use("java.util.Arrays").asList(certs);
                  };
                  if (DEBUG_MODE) console.log(`[+] ACP: ${className}->${methodName} (fallback X509TrustManager ext patch)`);
                } else {
                  if (DEBUG_MODE) console.log(`[*] ACP:[ ] Skipping unrecognized checkServerTrusted signature in class ${callingClass.class.getName()}`);
                }
              } else if (isMetaPinningMethod(errorMessage, failingMethod)) {
                failingMethod.implementation = function(certs) {
                  if (DEBUG_MODE) console.log(`[+] ACP: => Fallback patch for meta proxygen pinning`);
                  ...
                  if (DEBUG_MODE) {
                    console.log("[!] ACP: Meta unpinning fallback found only untrusted certificates");
                  }
                  return failingMethod.call(this, certs);
                };
                if (DEBUG_MODE) console.log(`[+] ACP: ${className}->${methodName} (Meta proxygen pinning fallback patch)`);
              } else {
                if (DEBUG_MODE) console.log("[!] ACP: [ ] Unrecognized TLS error - this must be patched manually");
                return;
              }
            });
          } catch (e) {
            if (DEBUG_MODE) console.log("[!] ACP: [ ] Failed to automatically patch failure");
            if (DEBUG_MODE) console.log(`[!] ACP: ${e}`);
          }
      ...
      if (DEBUG_MODE) console.log("[*] ACP: Unpinning fallback auto-patcher installed");
    } catch (err) {
      if (DEBUG_MODE) console.log(`[!] ACP: ${err}`);
      if (DEBUG_MODE) console.log("[!] ACP: Unpinning fallback auto-patcher installation failed");
    }
  });
}

// agent/anticertpinning/config_custom.js
var DEBUG_MODE = false;
var CERT_PEM = `-----BEGIN CERTIFICATE-----

+v6OV4e7pzjq
-----END CERTIFICATE-----`;
var PROXY_HOST = "192.168.179.2";
var PROXY_PORT = 8080;
...
if (DEBUG_MODE) {
  if (globalThis.Java?.available) {
    frida_java_bridge_default.perform(() => {
      setTimeout(() => console.log("[*] ACP: Scripts completed\n"), 5);
    });
  } else {
    setTimeout(() => console.log("[*] ACP: Scripts completed\n"), 5);
  }
...
new ApiResolver("module").enumerateMatches("exports:linker*!*dlopen*").forEach((dlopen) => {
  Interceptor.attach(dlopen.address, {
    ...
    onLeave(retval) {
      ...
      let module = Process.findModuleByName(this.moduleName) ?? Process.findModuleByAddress(retval);
      if (!module) {
...
var getAnonymousModule = (name, path, handle) => {
  const dlsymAddr = Module.findGlobalExportByName("dlsym");
  if (!dlsymAddr) {
    if (DEBUG_MODE) console.log(`[ACP] [!] Cannot find dlsym, cannot get anonymous module info for ${name}`);
    return;
  }
...
nativeConnectHook();
nativeTLSHook();
androidProxyOverride();
androidSystemCertInjection();
androidCertUnpinning();
androidCertUnpinningFallback();
androidDisableRootDetection();
export {
  BLOCK_HTTP3,
  CERT_DER,
  CERT_PEM,
